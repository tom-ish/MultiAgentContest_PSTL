...
public class MapSimulation extends AbstractSimulation{
	...
	public void initializeSimpleSimulation() {
		...
		ArrayList<Integer> agentPositions = new ArrayList<Integer>(); //used to store the agent's positions to ensure symmetric placement
		for (int i = 0; i < agents.length; i++) {
			MapSimulationAgent simAgent = (MapSimulationAgent) agents[i];
			simAgent.initialize(config);
			state.addAgent( (MapSimulationAgentState)simAgent.getAgentState(), agentPositions,
					 i < agents.length/2);
